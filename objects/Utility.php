<?php
	...
	class Utility {
		...
		public function callGetApi($url) {
			...
			curl_setopt($ch, CURLOPT_PROXY, '172.31.100.14');
		...
		// public function getPhobiaUser($categories) {
		// 	$username = $utility->string_encrypt_decrypt($_SESSION['isLoggedIn']['username'], false);
		// 	$query = "SELECT tags.hash FROM tags, users_phobia, tags_phobia, users WHERE users.username = :username AND users.id = users_phobia.user_id AND tags_phobia.id = users_phobia.phobia_id AND tags_phobia.tag_id = tags.id AND tags.is_deleted = 0 AND tags_phobia.is_deleted = 0 AND users_phobia.is_deleted = 0 AND users.is_deleted = 0";
		// 	$db = new DB();
		// 	$conn = $db->getConnection();
		// 	$stmt = $conn->prepare($query);
		// 	$stmt->bindParam(":username", $username);
		...
		// 		array_push($arr, $row['hash']);
		// 	}
		// 	$newArr = array();
		// 	for($i = 0; $i < count($categories); $i++) {
		// 		if(!in_array($categories[$i], $arr))
		// 			array_push($newArr, $categories[$i]);
		// 	}
		// 	return $newArr;
		// }
		public function getVenuesListUsingCategories($lat, $lng, $category, $radius=20000){
			...
			foreach($output['response']['venues'] as $i){
				...
				if(isset($i['id']))
					$venue['id'] = $i['id'];
				if(isset($i['name']))
					$venue['name'] = $this->reduceString($i['name'], 40);
		...
		public function getVenuesListUsingCityName($city, $category, $radius = 20000){
			...
			foreach($output['response']['venues'] as $i){
				...
				if(isset($i['id']))
					$venue['id'] = $i['id'];
				if(isset($i['name']))
					$venue['name'] = $this->reduceString($i['name'], 45);
				if(isset($i['location']['address']))
					$venue['address'] = $i['location']['address'];
				if(isset($i['location']['city']))
					$venue['city'] = $i['location']['city'];
	    ...
	    public function getDistanceAndTime($source, $destinations, $mode = "dri"){
	        ...
	        $url = $url."&key=AIzaSyDs4RkPfUAdMwio39BVawhkcJ_saRHp-n4";
	        // AIzaSyDpqF4zAPYUpQceScpqxyqu_XhCfy-Q1Uw
	        // AIzaSyC_lQzvumloLxq_y9loj7iYTbeHrDPZjx4
	        // AIzaSyAgQTvOhTgaw6XbP-LYSIUH9-HknmTXhwI
	        // echo $url;
